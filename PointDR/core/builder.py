...
def make_model() -> nn.Module:
    if configs.model.name == 'minkunet_dr':
        ...
        from core.models.semantic_kitti import MinkUNetV2
        if 'cr' in configs.model:
            cr = configs.model.cr
        else:
            cr = 1.0
        model = MinkUNetV2(num_classes=configs.data.num_classes, cr=cr)
        ...
        if 'cr' in configs.model:
            ...
            cr = 1.0
        model = MinkUNetV3(num_classes=configs.data.num_classes, cr=cr)
