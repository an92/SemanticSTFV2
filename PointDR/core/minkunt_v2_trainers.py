...
from PointDR.core.datasets.transform_3d import apply_entropy_guided_point_drop_batch
...
class MinkUnetV2Trainer(Trainer):
    ...
    def __init__(
        ...
        self.proto_loss_weight = lambda_proto
    ...
    def _run_step(self, feed_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with amp.autocast(enabled=self.amp_enabled):
            ...
            if self.model.training:
                # 传入 targets 以计算 proto loss
                voxel_logits, _, aux_losses = self.model(inputs, targets)

                # 1. Cross Entropy Loss
                ce_loss = self.criterion(voxel_logits, targets)

                # 2. Prototype Loss (从模型返回的字典中获取)
                proto_loss = aux_losses.get('proto_loss', torch.tensor(0.0, device=ce_loss.device))

                # Total Loss
                loss = ce_loss + self.proto_loss_weight * proto_loss

                # Log
                self.summary.add_scalar('loss', loss.item())
                self.summary.add_scalar('ce_loss', ce_loss.item())
                self.summary.add_scalar('proto_loss', proto_loss.item())
                ...
                return {'outputs': voxel_logits, 'targets': targets}

            else:
                with torch.no_grad():
                    voxel_logits, _, _ = self.model(inputs)
                ...
                for idx in range(invs.C[:, -1].max() + 1):
                    ...
                    outputs_mapped = voxel_logits[cur_scene_pts][cur_inv].argmax(1)
