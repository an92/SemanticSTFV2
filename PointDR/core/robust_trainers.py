...
def decorrelation_loss(features: torch.Tensor) -> torch.Tensor:
    """
    计算去相关性损失 (Decorrelation Loss)。
    最小化特征协方差矩阵的非对角线元素的平方和。
    """
    # 确保特征维度 N > 1 (至少两个点才能计算协方差)
    N = features.size(0)
    if N <= 1:
        return torch.tensor(0.0, device=features.device, dtype=features.dtype)

    # 1. 特征中心化 (Center the features)
    features = features - torch.mean(features, dim=0, keepdim=True)

    # 2. 计算协方差矩阵 C = (F^T F) / (N - 1)
    C = (features.T @ features) / (N - 1)

    # 3. 损失为所有非对角线元素的平方和
    # Loss = sum(C^2) - sum(diag(C)^2)
    diag = torch.diag(C)
    L_decorr = torch.sum(C**2) - torch.sum(diag**2)
    return L_decorr
